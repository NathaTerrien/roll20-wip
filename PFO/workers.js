    ...
    var update_spellsdc = function (attr) {
        ...
        if (maxlvl < 100) {
            console.log("*** DEBUG update_spellsdc min / max:" + minlvl + " / " + maxlvl);
            getAttrs(fields, function(v) {
                var update = {};
                var i = 0;
                for (i = minlvl; i < maxlvl; i++) {
                    console.log("*** DEBUG update_spellsdc i:" + i);
    ...
    var update_spells = function(level,update_id, source) {
        ...
        if(update_id.substring(0,1) === "-" && update_id.length === 20) {
            ...
            console.log("*** DEBUG update_spells current lvl: " + level);
            getSectionIDs("repeating_spell-" + level, function(idarray) {
                if(update_id === "all") {
                    ...
                    getAttrs(spell_attribs, function(v) {
                        var attr_spell_ids = [];
                        _.each(idarray, function(id) {
                            if((v["repeating_spell-" + level + "_" + id + "_spellatktype"] && v["repeating_spell-" + level + "_" + id + "_spellatktype"].includes(update_id))) {
                                attr_spell_ids.push(id);
                            }
                        });
                        if(attr_spell_ids.length > 0) {
                            do_update_spell(level,attr_spell_ids,source);
                        }
                    });
                };
            });
    ...
    var do_update_spell = function(spell_level,spell_array, source) {
        ...
        getAttrs(spell_attribs, function(v) {
            _.each(spell_array, function(spellid) {
                ...
                // Save DC
                update["repeating_spell-" + spell_level + "_" + spellid + "_spelldc"] = (parseInt(v["spells_dc_level_" + spell_level]) || 0) + (parseInt(v["repeating_spell-" + spell_level + "_" + spellid + "_spelldc_mod"]) || 0);
                // == update
                setAttrs(update, {silent: true});
            });
        });
